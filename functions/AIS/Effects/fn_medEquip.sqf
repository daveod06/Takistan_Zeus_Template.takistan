...
if (_healer call AIS_fnc_isMedic) then {
	_defi_pos = _healer modelToWorld [-0.5,0.2,0];
	_defi = "Land_Defibrillator_F" createVehicle _defi_pos;
	[_defi, [_defi_pos select 0, _defi_pos select 1, 0]] call AIS_fnc_setPosAGLS;
	...
	if (damage _injured >= 0.5 && {(random 2) >= 1}) then {
		...
		[_bb, [_bb_pos select 0, _bb_pos select 1, 0]] call AIS_fnc_setPosAGLS;
...
for "_i" from 1 to (1 + (round random 3)) do {
	...
	[_band, [_band_pos select 0, _band_pos select 1, 0]] call AIS_fnc_setPosAGLS;
...
if (random 2 >= 1) then {
	...
	[_ab, [_ab_pos select 0, _ab_pos select 1, 0]] call AIS_fnc_setPosAGLS;
