...
if (!(_unit getVariable ["ais_stabilized", false])) exitWith {
	...
	[{call AIS_fnc_bloodSplatterScreen}] call AIS_fnc_onNextFrame;
	
	// get the revive time
	_revive_time = [_unit] call AIS_fnc_calculateLifeTime;
	_unit setBleedingRemaining _revive_time;
	
	// loop
	private _acc_time = diag_tickTime + 1;
	[{diag_tickTime >= (_this select 2)}, {_this call AIS_fnc_bleeding}, [_unit, _revive_time, _acc_time]] call AIS_fnc_waitUntilAndExecute;
};
...
[{diag_tickTime >= (_this select 1)}, {_this call AIS_fnc_stabil}, [_unit, _acc_time]] call AIS_fnc_waitUntilAndExecute;