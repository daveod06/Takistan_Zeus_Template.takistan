/*
    ...
    * [object, caller, ID, arguments] is _this in the callback
	...
	Example:
	["Hint Hello!", player, 0, "true", { hint format ["Hello %1", name player]; }] call AIS_fnc_addAction;
	["Hint Hello!", "Man", 3, "true", { hint format ["Hello %1", name (_this select 0)]; }] call AIS_fnc_addAction;
*/
...
if (_distance > 0) then {
    _condition = format ["([cursorTarget, %1] call AIS_fnc_inRange) && {%2}", _distance, _condition];
};
