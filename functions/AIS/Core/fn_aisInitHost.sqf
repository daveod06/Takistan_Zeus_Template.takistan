...
if (local _unit && !(isPlayer _unit)) then {
	[_unit] spawn {
		...
		ais_hdEH = _unit addEventHandler ["HandleDamage", {_this call AIS_fnc_handleDamage}];
		
		ais_hkEH = _unit addEventHandler ["Killed", {_this call AIS_fnc_killed}];
		
		if ([_unit] call AIS_fnc_isPlayable) then {
			ais_hrEH = _unit addEventHandler ["Respawn", {_this call AIS_fnc_respawn}];
		};
		_unit removeAllEventHandlers "HandleHeal";
		ais_hhEH = _unit addEventHandler ["HandleHeal", {_this call AIS_fnc_handleHeal}];
	};
};