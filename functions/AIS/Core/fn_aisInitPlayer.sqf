...
if (local _player) then {
	[_player] call AIS_fnc_setVariables;
	...
	[_player] spawn {
		...
		_player addEventHandler ["HandleDamage", {_this call AIS_fnc_handleDamage}];
		
		_player addEventHandler ["Killed", {_this call AIS_fnc_killed}];
		
		_player addEventHandler ["Respawn", {_this call AIS_fnc_respawn}];
		
		_player addEventHandler ["HandleHeal", {_this call AIS_fnc_handleHeal}];
		
		waitUntil {!isNull (findDisplay 46)};
		(findDisplay 46) displayAddEventHandler ["KeyDown", {_this call AIS_fnc_keyHandler}];
	};
};
...
if (!isServer && (count (units group player) > 1)) then {
	if (leader group _player isEqualTo _player) then {
		...
		[_player] spawn {
			...
			{[_x] call AIS_fnc_setVariables; true} count _groupWithoutPlayers;
			
			waitUntil {!isNil {_player getVariable "BIS_fnc_feedback_hitArrayHandler"} || {time > 0}};
			{
				_x removeAllEventHandlers "handleDamage";
				//["%1 --- add damageEH to groupmember of %3's group: %2", diag_ticktime, _x, _player] call BIS_fnc_logFormat;
				_x addEventHandler ["HandleDamage", {_this call AIS_fnc_handleDamage}];
				
				_x removeAllEventHandlers "Killed";
				_x addEventHandler ["Killed", {_this call AIS_fnc_killed}];
				
				_x removeAllEventHandlers "Respawn";
				if ([_x] call AIS_fnc_isPlayable) then {
					_x addEventHandler ["Respawn", {_this call AIS_fnc_respawn}];
				};
				
				_x addEventHandler ["HandleHeal", {_this call AIS_fnc_handleHeal}];
...
if (_player getVariable ["ais_unconscious", false]) then {
	[{[(_this select 0)] call AIS_fnc_setUnconscious}, [_player]] call AIS_fnc_onNextFrame;
};

inGameUISetEventHandler ["Action", "_this call AIS_fnc_overrideHealAction"];
