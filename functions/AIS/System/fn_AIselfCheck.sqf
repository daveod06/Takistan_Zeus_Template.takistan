...
if (!isNull (_injured_ai getVariable ["ais_hasHelper", objNull])) then {
	...
	[{diag_tickTime > (_this select 1)}, {[(_this select 0)] call AIS_fnc_AIselfCheck}, [_injured_ai, _acc_time]] call AIS_fnc_waitUntilAndExecute;
};
...
[{diag_tickTime > (_this select 2)}, {_this call AIS_fnc_findAIHelper}, [_injured_ai, objNull, _acc_time]] call AIS_fnc_waitUntilAndExecute;
