...
if (primaryWeapon _unit isEqualTo "") exitWith {
	["This action is only with a primary Weapon possible."] call AIS_fnc_dynamicText;
};
...
if (_unit call AIS_fnc_checkLauncher) exitWith {
	["You are not able to carry anyone else while carrying a launcher on your back."] call AIS_fnc_dynamicText;
};
