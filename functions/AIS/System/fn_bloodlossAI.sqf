...
params ["_unit"];
if (!canSuspend) exitWith {[_unit] spawn AIS_fnc_bloodlossAI};
if (!local _unit) exitWith {};


// AI help AI
[_unit] call AIS_fnc_AIselfCheck;
...
if (diag_tickTime > _ai_time_over) exitWith {[_unit] call AIS_fnc_goToDead};

if (_unit getVariable ["ais_stabilized", false]) then {
	waitUntil {!alive _unit || {!(_unit getVariable ["ais_unconscious", false])}};
};

if (!alive _unit) exitWith {_unit call AIS_fnc_restoreFaks};
