...
if (isNull _helper) then {
	...
	if (count _all_members > 0) then {
		...
		if (count _matching_members_array > 0) then {
			{if (_x call AIS_fnc_isMedic) exitWith {_helper = _x};true} count _matching_members_array;
	...
	if (isNull _helper) then {
		...
		if (count _allMenNear > 0) then {
			...
			if (count _matching_side_array > 0) then {
				{if (_x call AIS_fnc_isMedic) exitWith {_helper = _x};true} count _matching_side_array;
...
if (local _helper) then {
	...
	[{diag_tickTime > (_this select 2)}, {
		[_this select 0, _this select 1] remoteExecCall ["AIS_fnc_moveAIHelper", 2];	
	}, [_injured, _helper, _acc_time]] call AIS_fnc_waitUntilAndExecute;
};
